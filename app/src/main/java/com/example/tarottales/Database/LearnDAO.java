...
public class LearnDAO extends DBContext {
    ...
    //<editor-fold desc="Search card by name">
    @SuppressLint("Range")
    public List<TarotCard> getListCardBySearchName(String searchName) {
        List<TarotCard> result = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        
        // Search in name or otherName, case-insensitive
        String sql = "select * from " + TB_TAROTCARD + 
                     " where name like ? or otherName like ? or keyword like ?";
